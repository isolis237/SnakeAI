...
def main(cfg: AppConfig):
    n_envs = cfg.runner.n_envs
    episodes_per_env = cfg.runner.episodes_per_env
    obs_mode = "flat"
    grid_w, grid_h = cfg.env.grid_w, cfg.env.grid_h
    fps = cfg.runner.fps
    render = cfg.render.render
    cell_px = cfg.render.cell_px

    # ---- build envs ----
    env_cfg = Config(grid_w=grid_w, grid_h=grid_h, relative_actions=True)
    envs = [SnakeEnv(Rules(env_cfg), obs_mode=obs_mode) for _ in range(n_envs)]
    obss = [env.reset(seed=(cfg.dqn.seed or 0) + i) for i, env in enumerate(envs)]
    ...
    device = torch.device(cfg.dqn.device)
    ...
    while ep_left.sum() > 0:
        ...
        if len(active_idx) == 0:
            ...
            for i in fin_idx:
                obss[i] = envs[i].reset(seed=(cfg.dqn.seed or 0) + total_steps + i)
                dones[i] = False
                returns[i] = 0.0
                scores[i] = 0
            continue
        ...
        for j, i in enumerate(active_idx):
            ...
            if render:
                renderers[i].draw(step.info["snapshot"])
